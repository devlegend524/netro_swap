...
import { Container, Grid, Typography, Box, Button, InputBase } from '@mui/material'
...
const SwapPage = () => {
  ...
  const [liquidityError, setLiquidityError] = useState(true)
  ...
  const getQuote = async () => {
    ...
    const amount = ethers.utils.parseUnits(sellBalance, tradeInfo.from.decimals)
  ...
  const getBalance = async () => {
    ...
    if (tradeInfo.from.name === 'ETH') setBalanceFrom(ethers.utils.formatEther(balance))
  }

  const getTokenBalance = async () => {
    if (tradeInfo.from.name !== 'ETH') {
      const provider = new ethers.providers.Web3Provider(window.ethereum)
      const contractFrom = new ethers.Contract(tradeInfo.from.contractAddress, ABI, provider)
      ...
      const contractTo = new ethers.Contract(tradeInfo.from.contractAddress, ABI, provider)
  ...
  return (
    <>
        ...
        <Container maxWidth='md'>
          <Grid className='main-wrapper' container justifyContent='center'>
            <Grid className='swap-wrapper'>
              ...
              <Grid container direction='column' className='swap-form' rowSpacing={2}>
                <Grid item>
                  <Box className='token-box'>
                    <Grid container alignItems={'center'}>
                      <Button className='token-select' onClick={(e) => showModal('from')}>
                        <img src={tradeInfo.from.icon} alt='icon' className='token-icon' />
                        {tradeInfo.from.name}
                        <ArrowDropDownIcon />
                      </Button>
                    ...
                    <Grid container justifyContent={'space-between'}>
                      ...
                      <Grid>
                        {balanceError && (
                          <Typography textAlign={'right'} className='balance-error'>
                ...
                <Grid item>
                  <Box className='token-box'>
                    <Grid container alignItems={'center'}>
                      <Button className='token-select' onClick={(e) => showModal('to')}>
                        <img src={tradeInfo.to.icon} alt='icon' className='token-icon' />
                        {tradeInfo.to.name}
                        <ArrowDropDownIcon />
                      </Button>
                      <InputBase
                ...
                <Grid>
                  ...
                  <Typography className='exchange-rate' textAlign={'right'}>
                    {pairResult.length > 0 ? (
                      <>
                        1 {pairResult[0].baseToken.symbol} = {pairResult[0].priceNative}{' '}
                ...
                <Grid item container justifyContent={'center'}>
                  {liquidityError ? (
                    ...
                    <>
                      {isConnected ? (
                        <>
                          {!bestProtocal || balanceError || !Number(sellBalance) ? (
                            <Button variant='contained' disabled className='swap-button' fullWidth>
                          ...
                          <tr className='dex-item' key={index}>
                            ...
                            <td>
                              {(
                                Number(
                                  ethers.utils
                                    .formatUnits(element.toTokenAmount, tradeInfo.to.decimals)
                                    .toString()
                                ) / Number(sellBalance)
                              ).toFixed(4)}
                            </td>
                            <td>
                              {index === 0 ? (
                                <Typography className='badge best'>Best</Typography>
                              ) : (
      ...
      />
      <SlippageModal modalOpen={slippageModalOpen} closeModal={() => setSlippageModalOpen(false)} />
    </>
  )
}
